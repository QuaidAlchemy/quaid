...
def create(filename: str):
    ...
    """
    from quaid.alchemy.schema.fec import FreeEnergyCalculationFactory
...
@click.option(
    ...
    help="The name of the experimental protocol that should be associated with this Alchemy network.",
    type=click.STRING,
    default=None,
    show_default=True,
)
...
def plan(
    ...
    import openfe
    from quaid.alchemy.schema.fec import FreeEnergyCalculationFactory
    from quaid.alchemy.schema.prep_workflow import AlchemyDataSet
    ...
    if custom_network_file is not None:
        from quaid.alchemy.schema.network import CustomNetworkPlanner
...
def submit(
    ...
    from quaid.cli.utils import print_header, print_message
    from quaid.alchemy.schema.fec import FreeEnergyCalculationNetwork
    from quaid.alchemy.utils import AlchemiscaleHelper
...
def predict(
    ...
    from quaid.alchemy.predict import (
    ...
    absolute_layout.save(
        absolute_path,
        title=f"Quaid-Alchemy-Absolute-{result_network.dataset_name}",
        embed=True,
    )
    ...
    relative_layout.save(
        relative_path,
        title=f"Quaid-Alchemy-Relative-{result_network.dataset_name}",
        embed=True,
    )
    report_status.stop()
