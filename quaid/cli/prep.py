...
import click
from quaid.cli.utils import SpecialHelpOrder
...
def create(filename: str, core_smarts: str):
    ...
    """
    import rich
    from quaid.cli.utils import print_header
...
@prep.command(
    short_help="From a set of compounds with diverse chemistry, create individual clusters of compounds suitable for Quaid-Alchemy."
)
@click.option(
    "-l",
    "--ligands",
...
def alchemize(
    ...
    message = Padding(
        f"Loaded {len(input_ligands)} ligands from [repr.filename]{ligands}[/repr.filename]",
        (1, 0, 1, 0),
    )
    console.print(message)
    ...
    outsiders, alchemical_clusters = compute_clusters(
        input_ligands, outsider_number, console
    )
...
@prep.command(
    short_help="Create an AlchemyDataset by running the given AlchemyPrepWorkflow which will expand the ligand states and generate constrained poses suitable for Quaid-Alchemy."
)
@click.option(
    "-f",
    "--factory-file",
...
def run(
    ...
    processors: The number of processors which can be used to run the workflow in parallel. `auto` will use all
        cpus -1, `all` will use all or the exact number of cpus to use can be provided.
    TODO add support for reference ligands which can be used in the network later
    """
    import pathlib
    ...
    quaid_ligands = MolFileFactory(filename=ligands).load()

    message = Padding(
        f"Loaded {len(quaid_ligands)} ligands from [repr.filename]{ligands}[/repr.filename]",
        (1, 0, 1, 0),
    )
    console.print(message)
    ...
    if receptor_complex is None and structure_dir is not None:
        ...
        #TODO fix the searching of reference complexes
        ...
        ref_complex, largest_ligand = select_reference_for_compounds(
            ligands=quaid_ligands, references=reference_complex, check_openmm=True
        )
    ...
    ref_ligands = None
    message = Padding(
        f"Starting Quaid-Prep workflow with {processors} processors", (1, 0, 1, 0)
    )
    console.print(message)

    alchemy_dataset = factory.create_alchemy_dataset(
        dataset_name=dataset_name,
        ligands=quaid_ligands,
